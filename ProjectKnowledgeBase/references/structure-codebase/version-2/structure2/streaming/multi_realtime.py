...
class MultiSourceRealTimeProcessor(BaseProcessor):
    ...
    def _stop_all_threads_safe(self):
        ...
        for thread_name, timeout in threads_to_stop:
            ...
            if thread and thread.is_alive():
                ...
                if thread.is_alive():
                    print(f"  {thread_name} did not stop gracefully")
    ...
    def _close_all_streams(self):
        ...
        for source_id in source_ids:
            try:
                ...
                print(f"  Error closing stream {source_id}: {e}")
    ...
    def _close_all_trackers(self):
        ...
        for source_id, tracker in list(self.tracking_managers.items()):
            try:
                ...
                print(f"  Error cleaning up tracker {source_id}: {e}")
    ...
    def _close_all_loggers(self):
        ...
        for source_id, logger in list(self.image_loggers.items()):
            try:
                ...
                print(f"  Error cleaning up logger {source_id}: {e}")
    
    def _close_all_queues(self):
    ...
    def monitor_resource_usage(self):
        ...
        with self._state_lock:
            ...
            stream_count = len(self.stream_managers)
            logger_count = len(self.image_loggers)
        ...
        try:
            ...
            print(f"   Active sources: {len(active_count)}")
            print(f"   Stream managers: {len(stream_count)}")
            print(f"   Image loggers: {len(logger_count)}")
    ...
    def print_final_statistics(self):
        ...
        with self._state_lock:
            loggers = list(self.image_loggers.items())
        ...
        if self.image_logging_enabled:
            ...
            for source_id, logger in loggers:
    ...
    def setup_multi_source_logging(self, base_filename: str = None):
        ...
        try:
            ...
            print(f"   Multi-source image logging ENABLED for {setup_count} sources")
    ...
    def print_cctv_mapping(self):
        ...
        with self._state_lock:
            for source_id, config in self.source_configs.items():
                ...
                logger_status = " " if source_id in self.image_loggers else " "
        ...
        with self._state_lock:
            try:
                ...
                if source_id not in self.source_configs:
                    print(f"  No configuration found for source: {source_id}, creating default...")
                    default_config = {
    ...
    def _get_verified_violations(self, results: List[Dict]) -> List[Dict]:
        ...
        """
        print("   _get_verified_violations is deprecated - use violation_verified flag from TrackingManager")
        return [
            r for r in results 
            if r.get('mask_status') == 'no_mask' 
    ...
    def print_multi_source_logging_status(self):
        ...
        print("\n" + "="*60)
        print("  MULTI-SOURCE IMAGE LOGGING STATUS")
        print("="*60)
    ...
    def robust_process_multi_source_frame(self, source_id: str, frame: np.ndarray) -> List[Dict]:
        ...
        try:
            ...
            if not self._validate_frame(frame):
                print(f"  Invalid frame from {source_id}")
                return []
            ...
            try:
                ...
                print(f"  Frame resize error for {source_id}: {e}")
    ...
    def _safe_handle_violation_logging(self, source_id: str, frame: np.ndarray, results: List[Dict]):
        ...
        try:
            ...
            print(f"  Violation logging error for {source_id}: {e}")
    ...
    def _attempt_stream_recovery(self, source_id: str) -> bool:
        ...
        with self._state_lock:  # Add thread safety
            if source_id not in self.source_configs:
            ...
            try:
                ...
                self.remove_source(source_id, external_lock=True)  
                
                # Wait before reconnection
                time.sleep(self.stream_recovery_wait_time)
                
                # Re-add the source using saved configuration
                success = self.add_source(source_id, source_config, external_lock=True)
    ...
    def monitor_stream_health(self):
        with self._state_lock:
            managers = list(self.stream_managers.items())   # (id, manager) tuples
        unhealthy = []
        for source_id, sm in managers:
            try:
                if not sm.is_healthy() or sm.get_success_rate() < self.health_success_rate_threshold:
    ...
    def auto_health_monitoring(self):
        ...
        if hasattr(self, 'health_monitor_thread') and self.health_monitor_thread and self.health_monitor_thread.is_alive():
            print("  Health monitor thread already running")
            return
    ...
    def optimize_memory_usage(self):
        ...
        with self._state_lock:
            # Snapshot of queues and trackers
            queue_items = list(self.frame_queues.items())
            tracker_items = list(self.tracking_managers.items())
            verified_violations = getattr(self, 'verified_violations', [])

        # Clear frame queues (no lock needed during I/O)
        for source_id, queue in queue_items:
            while not queue.empty():
                try:
                    queue.get_nowait()
                except:
        ...
        for source_id, tracker in tracker_items:
            try:
                tracker.face_tracker.cleanup_old_tracks(
                    tracker.frame_count, tracker.max_track_age
                )
                if hasattr(tracker, '_cleanup_old_violation_tracks'):
                    tracker._cleanup_old_violation_tracks(current_time)
            except Exception as e:
                print(f"  Memory optimization error for {source_id}: {e}")

        # Clear verified violations history
        if verified_violations and len(verified_violations) > 100:
            with self._state_lock:
    ...
    def print_tracking_status(self):
        with self._state_lock:
            trackers = list(self.tracking_managers.items())
        print("\n" + "="*50)
        print("👤 MULTI-SOURCE TRACKING SYSTEM STATUS")
        print("="*50)
        for source_id, tracking_manager in trackers:
    ...
    def get_verification_stats(self) -> Dict[str, Any]:
        ...
        for source_id, tracker in trackers:
            try:
                ...
                print(f"  Error getting verification stats for {source_id}: {e}")
        return all_stats # or total_stats if we want to return that
    ...
    def print_all_sources_info(self):
        with self._state_lock:
            source_ids = list(self.active_sources)
        print("\n" + "="*60)
        ...
        for source_id in source_ids:
            ...
            if info:
                    ...
                    print(f"      Logging: {info['has_image_logger']}")
    ...
    def setup_logging(self, filename: str = None):
        ...
        if len(self.active_sources) == 0:
            print("  No active sources for logging setup")
            return False
    ...
    def _extract_cctv_name_from_url(self, url: str, source_id: str = None) -> str:
        ...
        try:
            ...
            if self.debug_mode:
                print(f"  Error extracting CCTV name from URL: {e}")
    ...
    def set_display_size(self, width: int, height: int, method: str = " _size"):
        ...
        with self._state_lock:
            if hasattr(self, 'resizer'):
                ...
                print(f"   Display size set to {width}x{height} using {method} method")
    ...
    def print_verification_stats(self):
        ...
        print("="*60)
        with self._state_lock:
            stats = self.violation_stats.copy()        
        
        print(f"Total Detected: {stats.get('total_detected', 0)}")
        print(f"Total Verified: {stats.get('total_verified', 0)}")
        print(f"Total Logged: {stats.get('total_logged', 0)}")
        print(f"Verified Logged: {stats.get('verified_logged', 0)}")
        print(f"False Positives Prevented: {stats.get('false_positives_prevented', 0)}")
        
        if stats.get('last_verified_time'):
            last_time = datetime.datetime.fromtimestamp(stats['last_verified_time'])
            print(f"Last Verified: {last_time.strftime('%H:%M:%S')}")
        
        # Calculate rates
        total_detected = max(1, stats.get('total_detected', 1))
        verification_rate = (stats.get('total_verified', 0) / total_detected) * 100
        prevention_rate = (stats.get('false_positives_prevented', 0) / total_detected) * 100
    ...
    def add_source(self, source_id: str, url_or_config: Union[str, int, Dict] = None,
                external_lock: bool = False, **kwargs):
        if external_lock:
            return self._add_source_internal(source_id, url_or_config, **kwargs)
        with self._state_lock:
            return self._add_source_internal(source_id, url_or_config, **kwargs)
            
            
    def _add_source_internal(self, source_id: str, url_or_config, **kwargs):
        # Build configuration first
        final_config = self._create_source_config(url_or_config, **kwargs)

        # Check existence under lock (already held)
        if source_id in self.stream_managers:
            print(f"  Source {source_id} already exists")
            return False

        try:
            # Create stream manager and test connection
            stream_manager = StreamManager(final_config)
            if not stream_manager.initialize_stream(final_config['url']):
                print(f"  Failed to initialize stream for {source_id}")
                return False

            # Prepare tracking config
            global_tracking_config = self.config.get('tracking', {}) if hasattr(self, 'config') else {}
            source_tracking_config = final_config.get('tracking', {})
            tracking_config = self._deep_merge_tracking_config(
                global_tracking_config, source_tracking_config
            )
            if 'progressive_mask' not in tracking_config:
                tracking_config['progressive_mask'] = self._deep_merge_progressive_mask({}, {})

            # Create tracking manager (local, not yet stored)
            tracking_manager = TrackingManager(tracking_config)

            # Create frame queue
            frame_queue = Queue(maxsize=3)

            # --- All checks passed, now atomically update state ---
            self.source_configs[source_id] = final_config
            self.stream_managers[source_id] = stream_manager
            self.frame_queues[source_id] = frame_queue
            self.tracking_managers[source_id] = tracking_manager
            self.active_sources.append(source_id)

            # Start capture (after adding to dicts, but capture runs in background thread)
            ...
            traceback.print_exc()
            # No cleanup needed because we never stored anything
            return False
    ...
    def _setup_source_logging(self, source_id: str) -> bool:
        ...
        try:
            ...
            if source_logger.cctv_name != cctv_name:
                print(f"   CCTV name mismatch! Expected: {cctv_name}, Got: {source_logger.cctv_name}")
    ...
    def get_multi_source_frames(self, timeout: float = 0.1) -> Dict[str, Optional[np.ndarray]]:
        ...
        for source_id in source_ids:
            with self._state_lock:
                sm = self.stream_managers.get(source_id)
            if sm is None:
                frames[source_id] = None
                continue
            try:
                frame = sm.get_frame(timeout)
                frames[source_id] = frame
            except Exception as e:
                if self.config.get('debug', {}).get('verbose', False):
                    print(f"  Failed to get frame from {source_id}: {e}")
                frames[source_id] = None
        return frames
            ...
            while self.running and consecutive_errors < max_consecutive_errors:
                try:
                    ...
                    for source_id, frame in source_frames.items():
                        if frame is not None:
                            try:
                                ...
                                print(f"  Processing error for {source_id}: {e}")
                    ...
                    if display_frame is not None and display_frame.size > 0:
                        ...
                        print("  No display frame generated")
    ...
    def get_source_health_report(self) -> Dict[str, Dict]:
        with self._state_lock:
            managers = list(self.stream_managers.items())
        health_report = {}
        for source_id, sm in managers:
            health_report[source_id] = {
                'health': sm.get_stream_info(),
                'success_rate': sm.get_success_rate(),
    ...
    def stop_all_sources(self):
        ...
        if not self.running:
            print("  Already stopping or stopped")
            return
    ...
    def _handle_violation_logging(self, source_id: str, frame: np.ndarray, results: List[Dict]):
        ...
        if unverified_violations and self.debug_mode:
            print(f"   {len(unverified_violations)} unverified violations detected (not logged)")
