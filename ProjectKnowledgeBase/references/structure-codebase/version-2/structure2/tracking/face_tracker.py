...
class SimpleFaceTracker:
    ...
    # New public method for external track removal
    def remove_track(self, track_id: int) -> None:
        with self._lock:
            if track_id in self.tracks:
                del self.tracks[track_id]
                # also remove from deleted structures if needed
                if track_id in self.deleted_track_data:
                    del self.deleted_track_data[track_id]
                # remove from deque
                self.deleted_tracks = deque(
                    [item for item in self.deleted_tracks if item[0] != track_id],
                    maxlen=100
                )
                logger.debug(f"Track {track_id} manually removed")            