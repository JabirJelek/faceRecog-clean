...
function Initialize-ProjectPortablePaths {
    ...
    $Script:Config = @{
        ...
        EndTime = "11:48"    # Default end time
        ...
        PIDFilePath = Join-Path $dateBasedPath "monitor_pid_Laptop.json"
        MaxPIDFileAgeMinutes = 120
        
        # Logging - RELATIVE TO DATE-BASED PATH
        LogFile = Join-Path $dateBasedPath "monitor_Laptop.log"
...
function Update-ConfigForCurrentDate {
    ...
    $Script:Config.PIDFilePath = Join-Path $dateBasedPath "monitor_pid_Laptop.json"
    $Script:Config.LogFile = Join-Path $dateBasedPath "monitor_Laptop.log"
...
function Check-OrphanedRuns {
    ...
    if ($allRunFolders) {
        ...
        foreach ($folder in $allRunFolders) {
            ...
            $completionPath = Join-Path $folder.FullName "Laptop_Process_*\logs\completion_summary.txt"
