# script-organized-TowerCPU-0-resilient-monitor-fixed.ps1
# ==============================================
# Fixed version with working independent monitor
# ==============================================

# ========== CONFIGURATION ==========
$VENV_ROOT = "D:\RaihanFarid\Dokumen\faceRecog\.venv"
$PYTHON_SCRIPT = "D:\RaihanFarid\Dokumen\exp-magick\placeholder.py"
$POWERSHELL_SCRIPT_NAME = Split-Path -Leaf $MyInvocation.MyCommand.Path

# Python parameters
$PYTHON_PARAMS = @{
    "This-is-a-test" = $null
    "HTML Viewer" = "The Html"
}
...
$MONITOR_CONFIG = @{
    MonitorEnabled = $true
    CompletionFileName = "completion_summary.txt"
    CheckIntervalSeconds = 30
    MaxMonitorTimeHours = 24
...
function Start-IndependentMonitor {
    ...
    Write-ProcessLog "Creating independent monitor..." -Level "INFO"
    
    # Create monitor script content
...
'@
    ...
    $processInfo.Arguments = $powerShellArgs -join " "
    $processInfo.UseShellExecute = $false
    $processInfo.CreateNoWindow = $true
    $processInfo.RedirectStandardOutput = $true
    $processInfo.RedirectStandardError = $true
    
    $monitorProcess = New-Object System.Diagnostics.Process
    $monitorProcess.StartInfo = $processInfo
    
    try {
        $monitorProcess.Start() | Out-Null
        Write-ProcessLog "Independent monitor started with PID: $($monitorProcess.Id)" -Level "SUCCESS"
        return $monitorProcess
    } catch {
        Write-ProcessLog "Failed to start monitor process: $_" -Level "ERROR"
        return $null
    }
}

function Check-ProcessCompletion {
    param(
...
$RUN_FOLDER_NAME = "Run_Process_Testing-script_$timestamp"
...
try {
    ...
    Check-ProcessCompletion -Process $process -OutputFile $PYTHON_OUTPUT_FILE
...
$duration = if ($Global:ProcessState.ProcessEndTime -and $Global:ProcessState.ProcessStartTime) {
    $durationObj = $Global:ProcessState.ProcessEndTime - $Global:ProcessState.ProcessStartTime
    "{0:D2}:{1:D2}:{2:D2}" -f $durationObj.Hours, $durationObj.Minutes, $durationObj.Seconds
} else {
    "N/A"
}
