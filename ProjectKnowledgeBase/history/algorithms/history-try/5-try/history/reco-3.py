...
class FaceRecognitionSystem:
    def __init__(self, config: Dict):
        ...
        # Enhanced debug stats with mask detection metrics
        self.debug_stats = {
            'total_frames_processed': 0,
            'total_faces_detected': 0,
            'total_faces_recognized': 0,
            'total_masked_faces': 0,
            'faces_too_small_for_mask': 0,  
            'mask_detection_attempts': 0,   
            'mask_detection_success': 0,    
            'detection_times': [],
            'embedding_times': [],
            'recognition_times': [],
    ...
    def detect_mask_scale_aware(self, face_roi: np.ndarray, detection_confidence: float) -> Tuple[bool, float]:
        """
        PRIMARY MASK DETECTION METHOD - Scale-aware for RTSP cameras
        REPLACES: detect_mask_onnx_fixed, detect_mask_fixed
        """
        if self.mask_session is None or face_roi.size == 0:
            return False, 0.0
        
        # Calculate face size and apply scale-aware threshold
        h, w = face_roi.shape[:2]
        face_size = min(h, w)
        
        # Skip mask detection for very small faces
        min_face_size = self.config.get('min_face_size_for_mask', 80)
        if face_size < min_face_size:
            self.debug_stats['faces_too_small_for_mask'] += 1
            print(f"⚠️  Face too small for mask detection: {face_size}px < {min_face_size}px")
            return False, 0.0
        
        # Adjust confidence threshold based on face size and detection confidence
        base_threshold = self.config.get('mask_confidence_threshold', 0.7)
        if face_size < 100:
        ...
        self.debug_stats['mask_detection_attempts'] += 1
        
        try:
            resized_face = cv2.resize(face_roi, (224, 224))
            processed_faces = self._enhance_face_for_mask_detection(resized_face)
            
            best_confidence = 0.0
            best_result = False
            
            for processed_face in processed_faces:
                rgb_face = cv2.cvtColor(processed_face, cv2.COLOR_BGR2RGB)
                ...
                if mask_confidence > best_confidence:
                    best_confidence = mask_confidence
                    best_result = mask_confidence > adjusted_threshold
            
            if best_result:
                self.debug_stats['mask_detection_success'] += 1
            
            print(f"🔍 Scale-aware mask: size={face_size}px, conf={best_confidence:.3f}, threshold={adjusted_threshold:.3f}, result={best_result}")
            
            return best_result, float(best_confidence)
            
        except Exception as e:
            print(f"❌ Scale-aware mask detection error: {e}")
            return False, 0.0

    def _enhance_face_for_mask_detection(self, face_roi: np.ndarray) -> List[np.ndarray]:
        """Generate multiple enhanced versions of face for better mask detection"""
        enhanced_faces = [face_roi]  # Start with original
        ...
        if len(face_roi.shape) == 3:
            try:
                ...
                enhanced_faces.append(equalized)
            except Exception as e:
                print(f"⚠️  Histogram equalization failed: {e}")
        
        # Sharpening filter
        try:
            kernel = np.array([[-1, -1, -1], [-1, 9, -1], [-1, -1, -1]])
            sharpened = cv2.filter2D(face_roi, -1, kernel)
            enhanced_faces.append(sharpened)
        except Exception as e:
            print(f"⚠️  Sharpening failed: {e}")
        
        return enhanced_faces

    def analyze_face_sizes(self, detections: List[Dict], frame_shape: Tuple[int, int]) -> Dict:
        """Analyze face sizes and provide mask detection recommendations"""
        h, w = frame_shape
        face_sizes = []
        ...
        analysis = {
            ...
            'min_size': min(face_sizes) if face_sizes else 0,
            'max_size': max(face_sizes) if face_sizes else 0,
            'avg_size': np.mean(face_sizes) if face_sizes else 0,
            'suitable_for_mask_detection': 0,
            'recommendations': []
        }
        
        min_face_size = self.config.get('min_face_size_for_mask', 80)
        suitable_count = len([size for size in face_sizes if size >= min_face_size])
        analysis['suitable_for_mask_detection'] = suitable_count
        
        # Generate recommendations
        if analysis['avg_size'] < 60:
            analysis['recommendations'].append("🚨 Faces too small - consider moving camera closer")
            analysis['recommendations'].append(f"   Current avg: {analysis['avg_size']:.1f}px, Needed: ≥80px")
        elif analysis['avg_size'] < 100:
            analysis['recommendations'].append("⚠️  Marginal face size - mask detection may be unreliable")
        else:
            analysis['recommendations'].append("✅ Good face size for mask detection")
        
        return analysis

    def process_frame(self, frame: np.ndarray) -> List[Dict]:
        """
        PRIMARY PROCESSING METHOD - Uses scale-aware mask detection
        REPLACES: process_frame_with_mask, process_frame_scale_aware
        """
        ...
        # Analyze face sizes for mask detection suitability
        face_analysis = self.analyze_face_sizes(detections, frame.shape)
        if face_analysis['total_faces'] > 0 and self.mask_detector_enabled:
            print(f"📊 Face Analysis: {face_analysis['suitable_for_mask_detection']}/{face_analysis['total_faces']} suitable for mask detection")
            if face_analysis['recommendations']:
                for rec in face_analysis['recommendations']:
                    print(f"   {rec}")
        
        for detection in detections:
            x1, y1, x2, y2 = detection['bbox']
            
            # Apply padding and extract ROI
            ...
            # SCALE-AWARE MASK DETECTION
            is_wearing_mask = False
            mask_confidence = 0.0
            if self.mask_session is not None and self.mask_detector_enabled:
                is_wearing_mask, mask_confidence = self.detect_mask_scale_aware(
                    face_roi, 
            ...
            results.append({
                ...
                'face_size': min((x2_pad - x1_pad), (y2_pad - y1_pad))
            })
    ...
    def _load_mask_detector_onnx_fixed(self):
        """Load mask detector with NHWC format fix - PRIMARY METHOD"""
    ...
    # KEEP ALL OTHER METHODS (they are unique)
    ...
    def _load_embeddings_database(self):
        """Load pre-computed face embeddings from JSON"""
        try:
    ...
    def get_debug_stats(self) -> Dict:
        """Enhanced performance statistics with mask detection metrics"""
        ...
        # Enhanced mask detection metrics
        if self.debug_stats['mask_detection_attempts'] > 0:
            stats['mask_detection_success_rate'] = (
                self.debug_stats['mask_detection_success'] / 
                self.debug_stats['mask_detection_attempts'] * 100
            )
        else:
            stats['mask_detection_success_rate'] = 0
            
        stats['faces_too_small_for_mask'] = self.debug_stats['faces_too_small_for_mask']
        stats['mask_detection_attempts'] = self.debug_stats['mask_detection_attempts']
        stats['mask_detection_success'] = self.debug_stats['mask_detection_success']
...
class RealTimeProcessor:
    def __init__(self, face_system, processing_interval: int = 5, buffer_size: int = 10):
        ...
        # Mask detection optimization
        self.mask_debug_mode = False
        self.mask_optimization_enabled = False
        
        print("🎯 Scale-aware mask detection ENABLED")                

    def optimize_for_mask_detection(self):
        """Optimize system specifically for mask detection"""
        print("🎯 Optimizing for mask detection...")
        
        # Increase processing resolution for better mask detection
        old_scale = self.current_processing_scale
        self.current_processing_scale = min(1.5, self.current_processing_scale * 1.2)
        
        # Process more frames for better mask detection
        old_interval = self.processing_interval
        self.processing_interval = max(1, self.processing_interval - 1)
        
        # Adjust dynamic adjustment parameters for mask detection
        self.target_face_size = 120  # Aim for larger faces
        self.min_face_size = 80      # Minimum for mask detection
        
        self.mask_optimization_enabled = True
        
        print(f"   - Processing scale: {old_scale:.2f} → {self.current_processing_scale:.2f}")
        print(f"   - Processing interval: 1/{old_interval} → 1/{self.processing_interval}")
        print(f"   - Target face size: {self.target_face_size}px")

    def get_mask_detection_status(self, results: List[Dict]) -> Dict:
        """Get comprehensive mask detection status"""
        total_faces = len(results)
        masked_faces = len([r for r in results if r.get('is_wearing_mask', False)])
        min_face_size = self.face_system.config.get('min_face_size_for_mask', 80)
        suitable_faces = len([r for r in results if r.get('face_size', 0) >= min_face_size])
        
        status = {
            'total_faces': total_faces,
            'masked_faces': masked_faces,
            'suitable_faces': suitable_faces,
            'mask_detection_rate': (masked_faces / suitable_faces * 100) if suitable_faces > 0 else 0,
            'effectiveness': "GOOD" if suitable_faces > 0 else "POOR"
        }
        
        return status

    def draw_mask_detection_info(self, frame: np.ndarray, results: List[Dict]):
        """Display mask detection status and recommendations"""
        if not self.mask_debug_mode:
            return
            
        mask_status = self.get_mask_detection_status(results)
        
        info_lines = [
            f"Mask Detection Status:",
            f"Faces: {mask_status['total_faces']} total, {mask_status['suitable_faces']} suitable",
            f"Masked: {mask_status['masked_faces']} detected",
            f"Rate: {mask_status['mask_detection_rate']:.1f}%",
            f"Effectiveness: {mask_status['effectiveness']}",
        ]
        
        # Add recommendations based on status
        if mask_status['suitable_faces'] == 0 and mask_status['total_faces'] > 0:
            info_lines.append("💡 Move camera closer")
        elif mask_status['mask_detection_rate'] < 10 and mask_status['suitable_faces'] > 0:
            info_lines.append("💡 Faces may be too distant")
        
        # Draw background
        overlay = frame.copy()
        start_y = frame.shape[0] - len(info_lines) * 25 - 20
        end_y = frame.shape[0] - 10
        cv2.rectangle(overlay, (frame.shape[1] - 350, start_y), 
                     (frame.shape[1] - 10, end_y), (0, 0, 0), -1)
        cv2.addWeighted(overlay, 0.7, frame, 0.3, 0, frame)
        
        # Draw text
        for i, line in enumerate(info_lines):
            y_position = start_y + 20 + (i * 20)
            color = (0, 255, 255) if "💡" in line else (255, 255, 255)
            cv2.putText(frame, line, (frame.shape[1] - 340, y_position),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)

    def draw_enhanced_results_with_mask(self, frame: np.ndarray, results: List[Dict], performance: Dict):
        """Draw results with scale-aware mask detection info"""
        ...
        for result in results:
            ...
            # COLOR CODING BASED ON MASK STATUS AND FACE SIZE
            min_face_size = self.face_system.config.get('min_face_size_for_mask', 80)
            
            if face_size < min_face_size:
                color = (128, 128, 128)  # GRAY for faces too small for mask detection
                label = f"Too Small ({face_size}px)"
            ...
            # ADD FACE SIZE AND MASK STATUS INDICATOR
            if self.mask_debug_mode:
                status_text = f"Size: {face_size}px"
                if face_size >= min_face_size:
        ...
        self.draw_mask_detection_info(frame, results)  # NEW
        
        if performance and self.show_performance_stats:
            self.draw_dynamic_adjustment_info(frame, performance)
            
        # Show ONNX model status
        self.draw_onnx_status(frame)
      


    def print_control_reference(self):
        """Print updated control reference with mask optimization controls"""
        print("\n" + "="*60)
        print("🎮 ENHANCED KEYBOARD CONTROLS - SCALE-AWARE MASK DETECTION")
        print("="*60)
        # ... [existing sections] ...
        
        print("\n😷 MASK DETECTION CONTROLS:")
        print("  'u' - Toggle mask detection")
        print("  'o' - Optimize for mask detection") 
        print("  'y' - Toggle mask detection debug")
        
        print("="*60)

    # Update the main processing loop to use scale-aware processing
    ...
    def draw_enhanced_results_with_mask(self, frame: np.ndarray, results: List[Dict], performance: Dict):
        """Complete implementation of the drawing method"""
        if self.original_frame_size is None:
            self.original_frame_size = frame.shape[:2]
        ...
        for result in results:
            ...
            if face_size < min_face_size:
                color = (128, 128, 128)  # GRAY for faces too small
                label = f"Too Small ({face_size}px)"
            elif is_wearing_mask:
                color = (255, 255, 0)  # CYAN for masked faces
                label = f"Masked ({mask_conf:.2f})" + (f" - {identity}" if identity else "")
            elif identity:
        ...
        # Draw other debug information
        self.draw_debug_info(frame, results)
        self.draw_detection_debug(frame, results)
        self.draw_resize_info(frame)
        self.draw_mask_detection_info(frame, results)
    ...
    def handle_key_controls(self, key: int, display_frame: np.ndarray = None):
        ...
        elif key == ord('o'):  # Optimize for mask detection
            self.optimize_for_mask_detection()
            
        elif key == ord('y'):  # Toggle mask detection debug
            self.mask_debug_mode = not getattr(self, 'mask_debug_mode', False)
            status = "ON" if self.mask_debug_mode else "OFF"
            print(f"🔍 Mask detection debug: {status}")                        
    ...
    def run(self, source: str = "0"):
        """Main loop with scale-aware mask detection"""
        try:
            self.initialize_stream(source)
            self.start_frame_capture()
            
            print("🎮 Starting with SCALE-AWARE MASK DETECTION")
            ...
            while self.running:
                ...
                if should_process:
                    # Process on dynamically resized frame USING SCALE-AWARE METHOD
                ...
                # ALWAYS apply display scaling to results
                ...
                # Enhanced drawing with mask detection info
                self.draw_enhanced_results_with_mask(display_frame, results, last_performance)  # CHANGED
                cv2.imshow('Dynamic Face Recognition System - Scale-Aware Mask Detection', display_frame)
                
                # Handle ALL key controls
...
# Enhanced CONFIG for scale-aware mask detection
CONFIG = {
    ...
    # SCALE-AWARE MASK DETECTION CONFIG
    'mask_model_path': r'D:\SCMA\3-APD\fromAraya\Computer-Vision-CV\3.1_FaceRecog\run_py\mask_detector2.onnx',
    'mask_confidence_threshold': 0.3,  # Lower threshold for RTSP/distant faces
    'enable_mask_detection': True,
    'min_face_size_for_mask': 80,  
}
