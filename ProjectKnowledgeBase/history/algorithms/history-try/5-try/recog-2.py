...
class FaceRecognitionSystem:
    def __init__(self, config: Dict):
        ...
        self.mask_detector = None  # Add mask detector
        self.embeddings_db = {}
        self.identity_centroids = {}
        
        # Add mask detection stats
        self.debug_stats = {
    ...
    def detect_mask(self, face_roi: np.ndarray) -> Tuple[str, float]:
        """Detect if person is wearing mask"""
        ...
        try:
            ...
            input_size = (224, 224)
            resized_face = cv2.resize(face_roi, input_size)
    ...
    def process_frame(self, frame: np.ndarray) -> List[Dict]:
        ...
        for detection in detections:
            ...
            # Continue with embedding extraction
            ...
            results.append({
                ...
                'mask_status': mask_status,  # NEW
                'mask_confidence': mask_confidence,  # NEW
                'identity': identity,
...
class RealTimeProcessor:
    ...
    def draw_results(self, frame: np.ndarray, results: List[Dict]):
        ...
        for result in results:
            ...
            mask_status = result.get('mask_status', 'unknown')  # NEW
            mask_conf = result.get('mask_confidence', 0.0)  # NEW
            ...
            # Optional: Draw mask status icon
            self.draw_mask_icon(frame, x1, y1, mask_status)
    ...
    def draw_mask_icon(self, frame: np.ndarray, x: int, y: int, mask_status: str):
        """Draw small mask status icon"""
        icon_size = 20
        icon_x = x
        icon_y = y - 30  # Above the bounding box
        
        if mask_status == "mask":
            # Draw green checkmark or mask icon
            cv2.rectangle(frame, (icon_x, icon_y), (icon_x + icon_size, icon_y + icon_size), 
                        (0, 255, 0), -1)
            cv2.putText(frame, "M", (icon_x + 5, icon_y + 15), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        elif mask_status == "no_mask":
            # Draw red X or no-mask icon
            cv2.rectangle(frame, (icon_x, icon_y), (icon_x + icon_size, icon_y + icon_size), 
                        (0, 0, 255), -1)
            cv2.putText(frame, "NM", (icon_x + 2, icon_y + 15), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
