...
class UnknownFaceEnroller:
    def __init__(self, config, face_system):
        self.config = config
        self.face_system = face_system
        self.unknown_faces = {}  # track_id -> face data
        self.auto_enroll_count = 0
        
    def update_unknown_faces(self, results, frame_count):
        """Track unknown faces across frames with enhanced debugging"""
        if not self.config.get('enable_auto_enrollment', True):
            return {}
            
        current_unknowns = {}
        
        for result in results:
            # Check if this is an unknown face with sufficient confidence
            is_unknown = (result['identity'] is None or result['identity'] == 'Unknown')
            has_confidence = result['detection_confidence'] > self.config.get('auto_enroll_min_confidence', 0.6)
            has_embedding = 'embedding' in result and result['embedding'] is not None
            
            if is_unknown and has_confidence and has_embedding:
                # Generate unique ID for this unknown face
                face_id = self._generate_face_id(result['bbox'])
                
                if face_id in self.unknown_faces:
                    # Update existing track
                    track = self.unknown_faces[face_id]
                    track['frame_count'] += 1
                    track['last_seen'] = frame_count
                    track['embeddings'].append(np.array(result['embedding']))
                    track['bbox'] = result['bbox']
                    track['detection_confidences'].append(result['detection_confidence'])
                    
                    # Debug output
                    if track['frame_count'] % 10 == 0:
                        print(f"🔄 Tracking unknown face {face_id}: {track['frame_count']} frames")
                    ...
                    self.unknown_faces[face_id] = {
                        ...
                        'bbox': result['bbox'],
                        'detection_confidences': [result['detection_confidence']]
                    }
                    print(f"🎯 New unknown face detected: {face_id}")
                
                current_unknowns[face_id] = self.unknown_faces[face_id]
        
        # Clean up old tracks
        self._cleanup_old_tracks(frame_count)
        return current_unknowns
       
    def _generate_face_id(self, bbox):
        """Enhanced face ID generation using position and size for better tracking"""
        x1, y1, x2, y2 = bbox
        center_x, center_y = (x1 + x2) // 2, (y1 + y2) // 2
        width, height = x2 - x1, y2 - y1
        
        # Use grid-based positioning for more stable tracking
        grid_size = 50  # pixels
        grid_x = center_x // grid_size * grid_size
        grid_y = center_y // grid_size * grid_size
        size_bucket = (width // 20) * 20  # bucket sizes by 20px
        
        return f"face_{grid_x}_{grid_y}_{size_bucket}"
    ...
    def _cleanup_old_tracks(self, frame_count):
        ...
        for face_id, track in self.unknown_faces.items():
            if frame_count - track['last_seen'] > max_gap:
                to_remove.append(face_id)
        
        for face_id in to_remove:
            del self.unknown_faces[face_id]
    
    def check_and_enroll_unknown_faces(self, frame_count, original_frame=None):
        """Check if any unknown faces should be enrolled with enhanced logging"""
        if not self.config.get('enable_auto_enrollment', True):
            return 0
            
        # Check enrollment limits
        max_enroll = self.config.get('max_auto_enroll_per_session', 10)
        if self.auto_enroll_count >= max_enroll:
            print(f"⏹️ Auto-enrollment limit reached ({max_enroll})")
            return 0
            
        faces_to_enroll = []
        min_frames = self.config.get('auto_enroll_min_frames', 30)
        
        print(f"🔍 Checking {len(self.unknown_faces)} tracked unknown faces...")
        
        for face_id, track in self.unknown_faces.items():
            # Check duration threshold
            if track['frame_count'] >= min_frames:
                print(f"✅ Face {face_id} eligible: {track['frame_count']} frames (min: {min_frames})")
                
                # Verify low similarity with all existing identities
                if self._verify_low_similarity(track):
                    print(f"🎯 Face {face_id} passed similarity check - ready for enrollment!")
                    faces_to_enroll.append((face_id, track))
                else:
                    print(f"❌ Face {face_id} failed similarity check - matches existing identity")
            else:
                print(f"⏳ Face {face_id}: {track['frame_count']}/{min_frames} frames")
        
        # Enroll eligible faces
        enrolled_count = 0
        for face_id, track in faces_to_enroll:
            if self._enroll_new_face(track, original_frame):
                enrolled_count += 1
                self.auto_enroll_count += 1
                
                # Remove from tracking
                del self.unknown_faces[face_id]
                
                print(f"🎉 Successfully auto-enrolled {face_id}! Total: {self.auto_enroll_count}")
    ...
    def _verify_low_similarity(self, track):
        """Verify this face doesn't match any existing identity"""
        if not self.face_system.identity_centroids:
            return True  # No existing identities
        
        # Use the most recent embedding for verification
        recent_embedding = np.array(track['embeddings'][-1])
        
        # Check against all known identities
        threshold = self.face_system.config.get('recognition_threshold', 0.6)
        for identity, centroid in self.face_system.identity_centroids.items():
            similarity = self._compute_similarity(recent_embedding, centroid)
            if similarity > threshold:
                print(f"⚠️ Auto-enroll skipped: matches existing identity '{identity}' (similarity: {similarity:.3f})")
                return False  # Matches an existing identity
        
        return True  # No matches found

    def _compute_similarity(self, embedding1, embedding2):
        """Simple cosine similarity"""
        embedding1 = embedding1.flatten().astype(np.float64)
        embedding2 = embedding2.flatten().astype(np.float64)
        
        # Add epsilon to avoid division by zero
        eps = 1e-8
        norm1 = np.linalg.norm(embedding1) + eps
        norm2 = np.linalg.norm(embedding2) + eps
        
        return np.dot(embedding1, embedding2) / (norm1 * norm2)

    def _enroll_new_face(self, track, original_frame=None):
        """Create new identity in the database with proper structure"""
        try:
            # Generate unique name and ID
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            display_name = f"AutoEnrolled_{timestamp}"
            folder_name = self.config.get('auto_enroll_folder', 'AutoEnrolled')
            
            # Get the next available person ID
            existing_ids = list(self.face_system.embeddings_db['persons'].keys())
            next_id_num = len(existing_ids) + 1
            person_id = f"person_{next_id_num:03d}"
            
            # Use the most stable embedding (average of recent ones)
            ...
            # Create embedding entry in the correct format
            embedding_entry = {
                "vector": avg_embedding.tolist(),
                "source_file": f"auto_enrolled_face_{timestamp}.jpg",
                "file_path": f"auto_enrolled/{display_name}/auto_enrolled_face_{timestamp}.jpg",
            ...
            new_person = {
                ...
                "centroid_embedding": avg_embedding.tolist()
            }
            
            # Update in-memory database
            self.face_system.embeddings_db["persons"][person_id] = new_person
            
            # Update identity centroids for immediate recognition
            self.face_system.identity_centroids[display_name] = avg_embedding
            
            # Save to JSON file
            self._save_to_json()
            
            print(f"🎉 AUTO-ENROLLED: {display_name} (ID: {person_id})")
            print(f"   - Tracked for {track['frame_count']} frames")
            print(f"   - Embedding length: {embedding_length}")
            print(f"   - Avg detection confidence: {np.mean(track['detection_confidences']):.3f}")
            
            # Optional: Save the face image if original_frame is provided
            if original_frame is not None:
                self._save_face_image(display_name, track, original_frame)
            
            return True
            
        except Exception as e:
            print(f"❌ Failed to auto-enroll new face: {e}")
            import traceback
            traceback.print_exc()
            return False

    def _save_to_json(self):
        """Save the updated embeddings database to JSON file"""
        try:
            db_path = self.face_system.config['embeddings_db_path']
            with open(db_path, 'w') as f:
                json.dump(self.face_system.embeddings_db, f, indent=2)
            print(f"💾 Database updated: {db_path}")
        except Exception as e:
            print(f"❌ Failed to save database: {e}")

    def _save_face_image(self, display_name, track, original_frame):
        """Optional: Save the face image for reference"""
        try:
            # Create directory if it doesn't exist
            save_dir = Path("auto_enrolled_faces") / display_name
            ...
            if face_roi.size > 0:
                ...
                filename = save_dir / f"face_{timestamp}.jpg"
                cv2.imwrite(str(filename), face_roi)
                print(f"📸 Face image saved: {filename}")
                
        except Exception as e:
            print(f"⚠️ Could not save face image: {e}")

    def get_stats(self):
        """Get enrollment statistics"""
        return {
            'currently_tracking': len(self.unknown_faces),
            'auto_enrolled_count': self.auto_enroll_count,
            'max_auto_enroll': self.config.get('max_auto_enroll_per_session', 10)
        }
class ConfigManager:
...
class FaceRecognitionSystem:
    ...
    def get_identity_centroids(self):
        """Get current identity centroids for similarity checking"""
        return self.identity_centroids

    # Add this property for backward compatibility
    @property
    def identity_centroids(self):
        return self._identity_centroids

    @identity_centroids.setter  
    def identity_centroids(self, value):
        self._identity_centroids = value            
    ...
    def process_frame(self, frame: np.ndarray) -> List[Dict]:
        ...
        for detection in detections:
            ...
            # Extract embedding for ALL faces (crucial for auto-enrollment)
            embedding = self.extract_embedding(face_roi)
            if embedding is None:
                continue
                
            # Recognition (this might return None for unknown faces)
            ...
            # Return results with embedding for ALL faces
            results.append({
                'bbox': detection['bbox'],
                'detection_confidence': detection['confidence'],
                'mask_status': mask_status,  
                'mask_confidence': mask_confidence,  
                'identity': identity,  # This can be None for unknown faces
                'recognition_confidence': recognition_confidence,
                'embedding': embedding.tolist()  # Always include embedding
            })
...
class RobustFaceRecognitionSystem(FaceRecognitionSystem):
    def __init__(self, config: Dict):
        ...
        print("🎯 Robust Face Recognition with AUTO-ENROLLMENT support")
...
class RealTimeProcessor:
    def __init__(self, face_system, processing_interval: int = 5, buffer_size: int = 3):
        self.face_system = face_system
        self.unknown_enroller = UnknownFaceEnroller(face_system.config, face_system)        
        self.cap = None
        self.fps = 0
        self.frame_count = 0
    ...
    def handle_key_controls(self, key: int, display_frame: np.ndarray = None):
        ...
        elif key == ord('A'):  # Toggle auto-enrollment
            self.toggle_auto_enrollment()
            
        elif key == ord('S'):  # Print enrollment statistics  
            self.print_enrollment_stats()
            
        elif key == ord('R'):  # Reset enrollment counter
            self.reset_auto_enrollment_count()            
    ...
    def toggle_auto_enrollment(self):
        ...
        self.face_system.config['enable_auto_enrollment'] = new_setting
        self.unknown_enroller.config['enable_auto_enrollment'] = new_setting
        
        status = "ENABLED" if new_setting else "DISABLED"
        print(f"🎯 Auto-enrollment: {status}")
        return new_setting            

    def print_enrollment_stats(self):
        """Print current enrollment statistics"""
        stats = self.unknown_enroller.get_stats()
        print("\n" + "="*50)
        print("📊 AUTO-ENROLLMENT STATISTICS")
        print("="*50)
        print(f"Currently tracking: {stats['currently_tracking']} unknown faces")
        print(f"Auto-enrolled this session: {stats['auto_enrolled_count']}")
        print(f"Maximum allowed: {stats['max_auto_enroll']}")
        print(f"Feature enabled: {self.face_system.config.get('enable_auto_enrollment', True)}")
        print("="*50)

    def reset_auto_enrollment_count(self):
        """Reset the auto-enrollment counter"""
        old_count = self.unknown_enroller.auto_enroll_count
        self.unknown_enroller.auto_enroll_count = 0
        print(f"🔄 Auto-enrollment counter reset: {old_count} → 0")                          
    ...
    def print_control_reference(self):
        ...
        # Add to print_control_reference method
        print("\n🎯 AUTO-ENROLLMENT CONTROLS:")
        print("  'A' - Toggle auto-enrollment")
        print("  'S' - Print enrollment statistics") 
        print("  'R' - Reset enrollment counter")
        print("  Features:")
        print("    - Tracks unknown faces across frames")
        print("    - Auto-enrolls after 30+ frames with low similarity")
        print("    - Saves to JSON database in correct format")
        print("    - Prevents duplicates with existing identities")        
...
CONFIG = {
    ...
    'auto_enroll_min_frames': 30,           # Minimum frames to consider enrollment
    'auto_enroll_min_confidence': 0.6,      # Minimum detection confidence  
    'enable_auto_enrollment': True,         # Master switch
    'auto_enroll_folder': 'AutoEnrolled',   # Folder name for auto-enrolled faces
    'max_auto_enroll_per_session': 10,      # Prevent unlimited enrollment
...
def select_source():
    ...
    sources = {
        ...
        '4': 'rtsp://admin:admin@192.168.2.144:1934'
    }
