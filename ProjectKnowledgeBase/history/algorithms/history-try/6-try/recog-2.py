...
class RealTimeProcessor:
    ...
    # MODIFY the run method in RealTimeProcessor (critical section)
    def run(self, source: str = "0"):
        """Main loop with enhanced stability"""
        try:
            self.initialize_stream(source)
            self.start_frame_capture()
            
            print("🎮 Starting with STABILITY ENHANCEMENTS - Phase 1")
            ...
            while self.running:
                ...
                if original_frame is None:
                    time.sleep(0.005)  # Reduced sleep for responsiveness
                    continue
