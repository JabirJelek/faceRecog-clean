...
class FaceRecognitionSystem:
    ...
    def detect_mask(self, face_roi: np.ndarray) -> Tuple[str, float]:
        """Detect if person is wearing mask - EXACT framework replication"""
...
class RealTimeProcessor:
    ...
    def run(self, source: str = "0"):
        """Main loop with enhanced key controls - FIXED VERSION"""
        try:
            ...
            while self.running:
                ...
                if should_process:
                    ...
                    # ✅ SINGLE PLACE for dynamic adjustment
                    if self.dynamic_adjustment_enabled and self.frame_count % self.adaptive_check_interval == 0:
                        # Use scaled_results (in original coordinates) for performance analysis
                    ...
                    # ✅ SINGLE PLACE for debug frames
                    if self.save_debug_frames:
                        self.save_debug_frame(display_frame, scaled_results)
