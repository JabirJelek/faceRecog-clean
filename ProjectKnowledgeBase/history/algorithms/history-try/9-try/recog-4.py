...
class DensityBasedAlertManager(AlertManager):
    """
    Enhanced alert manager that triggers alerts based on violation density
    instead of individual violations
    """
    ...
    def __init__(self, config: Dict):
        ...
        self.violation_history = deque(maxlen=50)  # Track recent violations for analysis
        self.last_alert_violation_count = 0
        
        print(f"🎯 Density-Based Alerting initialized (threshold: {self.violation_threshold} violations)")
    
    def report_violations(self, violations: List[Dict]) -> bool:
        """
        Report violations and trigger alert only when threshold is reached
        ...
        # Add current violations to counter
        current_violations_count = len(violations)
        self.violation_counter += current_violations_count
        
        # Store violation details for message generation
        current_time = time.time()
        violation_batch = {
            'timestamp': current_time,
            'count': current_violations_count,
            'recognized_identities': [v.get('identity') for v in violations if v.get('identity') and v.get('identity') != 'Unknown'],
            'unknown_count': len([v for v in violations if not v.get('identity') or v.get('identity') == 'Unknown']),
            'total_violations': current_violations_count
        }
        self.violation_history.append(violation_batch)
        
        print(f"📊 Violation density: {self.violation_counter}/{self.violation_threshold} "
              f"(+{current_violations_count} new violations)")
        
        # Check if we've reached the threshold
        if self.violation_counter >= self.violation_threshold:
            return self._trigger_density_alert()
        
        return False
    
    def _trigger_density_alert(self) -> bool:
        """Trigger alert when violation density threshold is reached"""
        # Check cooldown first (inherited from parent)
        current_time = time.time()
        with self.alert_lock:
            if current_time - self.last_alert_time < self.cooldown_seconds:
                print(f"⏰ Density alert ready but cooldown active: "
                      f"{int(self.cooldown_seconds - (current_time - self.last_alert_time))}s remaining")
                return False
            self.last_alert_time = current_time
        
        # Generate intelligent message based on accumulated violations
        message = self._generate_density_message()
        ...
        # Reset counter after successful alert trigger
        self.last_alert_violation_count = self.violation_counter
        self.violation_counter = 0
        
        print(f"🚨 DENSITY ALERT TRIGGERED: {message}")
        print(f"   - Total violations accumulated: {self.last_alert_violation_count}")
        print(f"   - Counter reset to: {self.violation_counter}")
        
        return True
    
    def _generate_density_message(self) -> str:
        """Generate intelligent message based on accumulated violation patterns"""
        if not self.violation_history:
            return "Terdeteksi pelanggaran masker berulang"
        
        # Analyze recent violation patterns
        recent_violations = list(self.violation_history)
        
        total_accumulated = self.violation_counter
        all_recognized = []
        total_unknown = 0
        
        for batch in recent_violations:
            all_recognized.extend(batch['recognized_identities'])
            total_unknown += batch['unknown_count']
        
        # Remove duplicates from recognized identities
        unique_recognized = list(set(all_recognized))
        
        # Generate appropriate message
        if unique_recognized:
            names = ", ".join(unique_recognized[:3])  # Limit to 3 names
            if len(unique_recognized) > 3:
                message = (f"Perhatian! {names} dan {len(unique_recognized)-3} lainnya "
                          f"tidak memakai masker secara berulang ({total_accumulated} pelanggaran terdeteksi)")
            else:
                message = (f"Perhatian! {names} tidak memakai masker secara berulang "
                          f"({total_accumulated} pelanggaran terdeteksi)")
        else:
            if total_unknown == 1:
                message = "Perhatian! 1 orang tidak dikenal tidak memakai masker secara berulang"
            else:
                message = (f"Perhatian! {total_unknown} orang tidak dikenal "
                          f"tidak memakai masker secara berulang ({total_accumulated} pelanggaran terdeteksi)")
        
        # Add urgency for high density
        if total_accumulated >= 15:
    ...
    def get_density_stats(self) -> Dict:
        ...
        return {
            ...
            'progress_percentage': (self.violation_counter / self.violation_threshold) * 100,
            'recent_violation_batches': len(self.violation_history),
            'last_alert_violation_count': self.last_alert_violation_count,
            'cooldown_remaining': max(0, self.cooldown_seconds - (time.time() - self.last_alert_time))
        }
    
    def reset_density_counter(self):
        """Manually reset the density counter"""
        old_count = self.violation_counter
        self.violation_counter = 0
        self.violation_history.clear()
        print(f"🔄 Density counter reset: {old_count} → 0")
    
    def set_density_threshold(self, threshold: int):
        """Update the density threshold"""
        old_threshold = self.violation_threshold
        self.violation_threshold = max(1, threshold)  # Minimum 1
        print(f"📏 Density threshold updated: {old_threshold} → {self.violation_threshold}")
...
class RealTimeProcessor:
    def __init__(self, face_system, processing_interval: int = 5, buffer_size: int = 3):
        ...
        # 🆕 VOICE ALERT SYSTEM
        self.alert_manager = DensityBasedAlertManager(self.config)
        self.sent_alerts = set()  # Track alerted identities to avoid duplicates
        
        self.density_stats_history = deque(maxlen=20)
        print("🎯 Density-Based Alerting integrated into RealTimeProcessor")        
    ...
    def check_and_send_alerts(self, results: List[Dict]):
        ...
        # Use density-based reporting instead of immediate alerts
        if violations:
            alert_sent = self.alert_manager.report_violations(violations)
            
            # Log density statistics periodically
            if self.processing_count % 30 == 0:  # Every 30 processed frames
                stats = self.alert_manager.get_density_stats()
                self.density_stats_history.append(stats)
                self._log_density_stats(stats)
                
    def _log_density_stats(self, stats: Dict):
        """Log density statistics for monitoring"""
        progress = stats['progress_percentage']
        print(f"📊 Density Progress: {progress:.1f}% "
              f"({stats['violation_counter']}/{stats['violation_threshold']}) "
              f"| Recent batches: {stats['recent_violation_batches']}")
                        
    def get_alerting_stats(self) -> Dict:
        """Get comprehensive alerting statistics"""
        base_stats = super().get_debug_stats() if hasattr(super(), 'get_debug_stats') else {}
        density_stats = self.alert_manager.get_density_stats()
        
        return {
            **base_stats,
            'density_alerting': density_stats,
            'density_stats_history_size': len(self.density_stats_history)
        }
    ...
    def handle_key_controls(self, key: int, display_frame: np.ndarray = None):
        ...
        elif key == ord('D'):  # Toggle density alerting
            self.alert_manager.enabled = not self.alert_manager.enabled
            status = "ENABLED" if self.alert_manager.enabled else "DISABLED"
            print(f"🎯 Density alerting: {status}")
        
        elif key == ord('R'):  # Reset density counter
            self.alert_manager.reset_density_counter()
        
        elif key == ord('='):  # Increase density threshold
            current = self.alert_manager.violation_threshold
            self.alert_manager.set_density_threshold(current + 5)
        
        elif key == ord('_'):  # Decrease density threshold (using underscore key)
            current = self.alert_manager.violation_threshold
            self.alert_manager.set_density_threshold(max(1, current - 5))
        
        elif key == ord('S'):  # Show density statistics
            stats = self.alert_manager.get_density_stats()
            print("\n" + "="*50)
            print("📊 DENSITY ALERTING STATISTICS")
            print("="*50)
            print(f"Violation Counter: {stats['violation_counter']}/{stats['violation_threshold']}")
            print(f"Progress: {stats['progress_percentage']:.1f}%")
            print(f"Recent Batches: {stats['recent_violation_batches']}")
            print(f"Last Alert Count: {stats['last_alert_violation_count']}")
            print(f"Cooldown Remaining: {stats['cooldown_remaining']:.1f}s")
            print("="*50)                                    
    ...
    def print_control_reference(self):
        ...
        print("🎯 DENSITY ALERTING CONTROLS:")
        print("  'D' - Toggle density-based alerting")
        print("  'R' - Reset density counter")
        print("  '+' - Increase density threshold (+5)")
        print("  '_' - Decrease density threshold (-5)") 
        print("  'S' - Show density statistics")
        print()
        print("📊 CURRENT SETTINGS:")
        stats = self.alert_manager.get_density_stats()
        print(f"  Threshold: {stats['violation_threshold']} violations")
        print(f"  Current: {stats['violation_counter']} violations")
        print(f"  Progress: {stats['progress_percentage']:.1f}%")
        print("="*60)        
        print()
...
# Enhanced configuration with density-based alerting
DENSITY_ALERT_CONFIG = {
    **CONFIG,
    
    # Density-based alerting parameters
    'violation_threshold': 10,              # Alert every 10 violations
    'alert_cooldown_seconds': 60,           # 1 minute cooldown (can be shorter now)
    'enable_voice_alerts': True,
    
    # Keep existing alert server config
    'alert_server_url': 'https://your-domain.my.id/actions/a_notifikasi_suara_speaker.php',
}
...
def main_priority_optimized():
    ...
    face_system = RobustFaceRecognitionSystem(DENSITY_ALERT_CONFIG)
