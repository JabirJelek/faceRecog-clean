...
class MultiScaleFaceProcessor:
    def __init__(self, config: Dict):
        ...
        # OPTIMIZED: Reduced number of scales for marginal quality faces
        # Original: 7 scales × 7 rotations = 49 embeddings
        # Optimized: 3 scales × 3 rotations = 9 embeddings for marginal cases
        self.scale_factors_full = [0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75]
        self.scale_factors_light = [0.75, 1.0, 1.25]  # For marginal quality
        self.rotation_angles_full = [-15, -10, -5, 0, 5, 10, 15]
        self.rotation_angles_light = [-10, 0, 10]  # For marginal quality
        
        # Quality-based selection
        self.quality_mode_threshold = 0.5
    ...
    def extract_multi_scale_embeddings(self, face_roi: np.ndarray, 
                                       quality_score: float = 0.5) -> List[np.ndarray]:
        """Extract embeddings from multiple scales and slight rotations with quality-aware optimization"""
        embeddings = []
        h, w = face_roi.shape[:2]
        
        # OPTIMIZATION: Choose scale/rotation set based on quality
        # Lower quality gets lighter processing to balance computation vs benefit
        if quality_score < self.quality_mode_threshold:
            scale_factors = self.scale_factors_light
            rotation_angles = self.rotation_angles_light
            mode = "light"
        else:
            scale_factors = self.scale_factors_full
            rotation_angles = self.rotation_angles_full
            mode = "full"
        
        if self.config.get('verbose', False):
            print(f"Multi-scale mode: {mode} (Q={quality_score:.2f}) - {len(scale_factors)} scales × {len(rotation_angles)} rotations")
