...
from face_recog_modular1.streaming.stream_manager import StreamManager
from face_recog_modular1.streaming.base_processor import BaseProcessor
from face_recog_modular1.visualization.display_resizer import DisplayResizer
from face_recog_modular1.streaming.performance_manager import PerformanceManager
from face_recog_modular1.streaming.frame_utils import FrameUtils
from face_recog_modular1.streaming.control_handler import ControlHandler
from face_recog_modular1.tracking.tracking_manager import TrackingManager
from face_recog_modular1.alerting.alert_manager import DurationAwareAlertManager
from face_recog_modular1.logging.image_logger import ImageLogger
...
class MultiSourceRealTimeProcessor(BaseProcessor):
    def __init__(self, face_system, config: Dict):
        ...
        self.violation_verification_enabled = config.get('violation_verification_enabled', True)
        self.min_violation_duration = config.get('min_violation_duration', 0.3)
        self.min_violation_frames = config.get('min_violation_frames', 1)
        self.violation_confidence_threshold = config.get('violation_confidence_threshold', 0.3)
        ...
        self.processing_width = config.get('processing_width', 640)
        self.processing_height = config.get('processing_height', 480)
    ...
    def apply_verification_config(self, config: Dict):
        ...
        try:
            verification_config = {
                ...
                'min_violation_duration': config.get('min_violation_duration', 3.0),
                'min_violation_frames': config.get('min_violation_frames', 5),
