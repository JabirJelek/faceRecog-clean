...
class ProgressiveMaskDetector:
    ...
    def _initialize_config(self) -> None:
        ...
        self.confidence_threshold = max(0.0, min(1.0, 
            self.config.get('mask_confidence_threshold', 0.60)))
        self.consistency_threshold = max(0.0, min(1.0,
            self.config.get('mask_consistency_threshold', 0.55)))
        self.min_frames_for_decision = max(1,
            self.config.get('min_mask_frames', 5))
        self.occlusion_timeout = max(0.1,
            self.config.get('occlusion_timeout', 7))
        ...
        self.weight_increase_high_conf = max(0.0, min(1.0,
            self.config.get('weight_increase_high_conf', 0.0)))
        self.weight_decrease_low_conf = max(0.0, min(1.0,
            self.config.get('weight_decrease_low_conf', 0.0)))
        self.weight_increase_opposite = max(0.0, min(1.0,
            self.config.get('weight_increase_opposite', 0.0)))
        ...
        self.early_confidence_threshold = self.config.get('early_confidence_threshold', 0.9)
    ...
    def _update_label_weights(self, track_id: str, current_status: MaskStatus, 
        ...
        if frame_count < self.early_frame_threshold and raw_confidence > self.early_confidence_threshold:
            ...
            track.label_weights = {
                MaskStatus.MASK.value: 0.1,
                MaskStatus.NO_MASK.value: 0.9
            }
            return
        ...
        if raw_confidence >= 0.7:  # High confidence
    ...
    def _determine_status_from_analysis(self, temporal: Dict, confidence: Dict, 
        ...
        if track_id and track_id in self.tracks:
            ...
            effective_mask_ratio = mask_ratio * weights.get(MaskStatus.MASK.value, 0.01)
            effective_no_mask_ratio = no_mask_ratio * weights.get(MaskStatus.NO_MASK.value, 0.99)
            
            self.logger.debug(f"Weighted ratios: mask={effective_mask_ratio:.3f} "
                  f"(raw={mask_ratio:.3f}×w={weights.get(MaskStatus.MASK.value, 0.01):.2f}), "
                  f"no_mask={effective_no_mask_ratio:.3f} "
                  f"(raw={no_mask_ratio:.3f}×w={weights.get(MaskStatus.NO_MASK.value, 0.99):.2f})")
