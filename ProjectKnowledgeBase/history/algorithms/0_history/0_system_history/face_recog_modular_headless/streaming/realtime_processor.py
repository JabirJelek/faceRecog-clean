...
from face_recog_modular.visualization.display_resizer import DisplayResizer
from face_recog_modular.processing.scene_analysis import ContextAwareDynamicScaling
from face_recog_modular.tracking.tracking_manager import TrackingManager
from face_recog_modular.alerting.alert_manager import DurationAwareAlertManager
from face_recog_modular.logging.image_logger import ImageLogger

# Import new modular components
from face_recog_modular.streaming.base_processor import BaseProcessor
from face_recog_modular.streaming.performance_manager import PerformanceManager
from face_recog_modular.streaming.frame_utils import FrameUtils
from face_recog_modular.streaming.control_handler import ControlHandler
from face_recog_modular.streaming.stream_manager import StreamManager

class RealTimeProcessor(BaseProcessor):
    def __init__(self, face_system, processing_interval: int = 5, buffer_size: int = 3):
        # Initialize base class
        super().__init__(face_system, face_system.config)
        ...
        # Debug controls - ADD MISSING ATTRIBUTES
        self.debug_mode = False
        self.show_detection_debug = False
        self.show_performance_stats = False  # ADD THIS MISSING ATTRIBUTE
        ...
        # Individual trackers with unified TrackingManager
        self.tracking_manager = TrackingManager(face_system.config)
        ...
        print("👤 Unified TrackingManager INITIALIZED")
    ...
    def log_performance_data(self, results: List[Dict], display_frame: np.ndarray = None, original_frame: np.ndarray = None):
        """Enhanced logging with synchronized voice alerts using proper ImageLogger"""
        ...
        try:
            ...
            # 🆕 USE SYNCHRONIZED ALERTS with image logging
            if (self.image_logging_enabled and 
                self.image_logger.has_mask_violations(results)):
                
                print("🚨 ATTEMPTING TO SAVE VIOLATION IMAGE AND TRIGGER AUDIO")
    ...
    def save_annotated_frame(self, frame: np.ndarray, results: List[Dict], original_frame: np.ndarray = None) -> bool:
        ...
        success, base64_data = self.image_logger.save_annotated_frame(frame, results, original_frame)
        
        if success and base64_data:
            print(f"🖼️  Image saved with base64 data (length: {len(base64_data)})")
    ...
    def run(self, source: str = "0"):
        """Main loop using StreamManager for frame capture"""
        try:
            ...
            while self.running:
                ...
                if should_process:
                    ...
                    # 🎯 ENHANCED: Use tracking manager for comprehensive processing
    ...
    def get_stability_metrics(self) -> Dict:
        ...
        if hasattr(self, 'tracking_manager'):
            ...
            base_metrics.update({
                'tracking_active_tracks': tracking_stats['face_tracker'].get('active_tracks', 0),
                'tracking_total_tracks': tracking_stats['face_tracker'].get('total_tracks', 0),
    ...
    def print_stability_report(self):
        ...
        print("\n" + "="*50)
        print("📊 STABILITY REPORT - Modular System")
        print("="*50)
        ...
        if 'tracking_active_tracks' in metrics:
            print(f"Active Tracks: {metrics['tracking_active_tracks']}")
