...
class ImageLogger:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.image_log_interval = config.get('image_log_interval', 0)
        self.max_images_per_session = config.get('max_images_per_session', 0)
        self.min_save_interval = config.get('min_save_interval', 0.0)
        ...
        if self.server_endpoint:
            ...
            if not parsed.scheme or not parsed.netloc:
                self.logger.error(f"Invalid server endpoint URL in config: {self.server_endpoint}")
                self.server_push_enabled = False
                
        self.server_push_cooldown = config.get('server_push_cooldown', 0)  # seconds
        ...
        # Queue and worker
        self._push_queue = queue.Queue(maxsize=50)  # bounded queue
        self._worker_thread = threading.Thread(target=self._server_push_worker, daemon=True)
        self._worker_thread.start()        
        
        
        
    def _server_push_worker(self):
        """Background thread for processing server push requests."""
        while True:
            try:
                violation_data = self._push_queue.get(timeout=1)
                if violation_data is None:  # shutdown signal
                    break
                self.push_violation_to_server(violation_data)
            except queue.Empty:
                continue
            except Exception as e:
                self.logger.exception(f"Unhandled error in server push worker: {e}")      
        ...
        with self._lock:
            ...
            try:
                ...
                if success:
                    ...
                    # Push to server if enabled and we have base64 data
                    if self.server_push_enabled and base64_data:
                        ...
                        # Create comma-separated string of ALL detected names
                        if detected_names:
                        ...
                        # Prepare violation data with ONLY required fields
                        violation_data = {
                        ...
                        try:
                            self._push_queue.put_nowait(violation_data)
                            server_success = True  # optimistic; actual result handled asynchronously
                            self.logger.debug(f"Queued server push for {detected_name_str}")
                        except queue.Full:
                            self.logger.warning("Server push queue full, dropping violation")
    ...
    def update_server_config(self, server_config: Dict[str, Any]) -> None:
        ...
        """
        with self._lock:
            new_endpoint = server_config.get('server_endpoint', self.server_endpoint)
            if new_endpoint:
                parsed = urlparse(new_endpoint)
                if not parsed.scheme or not parsed.netloc:
                    self.logger.error(f"Invalid server endpoint URL: {new_endpoint}")
                    return        
            try:
