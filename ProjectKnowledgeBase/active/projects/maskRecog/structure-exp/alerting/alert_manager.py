...
class DurationAwareAlertManager:
    ...
    def __init__(self, config: Dict):
        ...
        self.batch_lock = threading.Lock()   # new lock for batch data    
        
        self.alert_executor = ThreadPoolExecutor(max_workers=5)    
    ...
    def _send_alert_thread(self, message: str, identity: str, mask_status: str):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(f"❌ Alert server returned status: {response.status_code}")
                
        except requests.exceptions.Timeout:
            self.logger.info(f"⏰ Alert request timed out after {timeout} seconds")
        except requests.exceptions.RequestException as e:
            self.logger.info(f"❌ Failed to send voice alert: {e}")
        except Exception as e:
            self.logger.info(f"❌ Unexpected error in alert thread: {e}")
    ...
    def send_voice_alert(self, message: str, identity: str = None, mask_status: str = None) -> bool:
        ...
        self.alert_executor.submit(self._send_alert_thread, message, identity, mask_status)
        return True
    ...
    def update_config(self, config: Dict):
        ...
        try:
            ...
            self.logger.info(f"❌ Error updating alert config: {e}")
