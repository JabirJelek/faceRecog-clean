...
.NOTES
Configured for faceRecog\process-run output structure
#>
...
function Initialize-ProjectPortablePaths {
    ...
    Write-Host "Venv Root: $VenvRoot" -ForegroundColor Green
...
function Get-ProjectConfiguration {
    ...
    return @{
        ...
        WorkerScript = Join-Path $ProjectRoot "patterns\scripts\1_laptop\maskDetect-portable.ps1"
        
        # Python script path - RELATIVE to project root  
        PythonScriptPath = Join-Path $ProjectRoot "patterns\algorithm\py-test.py"
        
        # Paths for validation - RELATIVE to active root
        RunsBasePath = Join-Path $ActiveRoot "logs-running\laptop"
        ...
        WorkerStartCooldown = 30  # New: Wait time before restarting worker if Python is still running
        MaxPythonRuntimeMinutes = 180  # New: Maximum expected runtime for Python process        
        
        # PID tracking - RELATIVE to RunsBasePath
        PIDFilePath = Join-Path (Join-Path $ActiveRoot "logs-running\laptop") "monitor_pid_Laptop.json"
        MaxPIDFileAgeMinutes = 120
        
        # Logging - RELATIVE to RunsBasePath
        LogFile = Join-Path (Join-Path $ActiveRoot "logs-running\laptop") "monitor_Laptop.log"
        ...
        SuddenTermination = @{
            TrackingFile = Join-Path (Join-Path $ActiveRoot "logs-running\laptop") "sudden_termination_tracking.json"
...
function Test-PythonProcessActivity {
    <#
    .SYNOPSIS
    Checks if a Python process is actually doing work, not just stuck
    #>
    param(
        [int]$PythonPID
    )
    
    if (-not $PythonPID -or $PythonPID -eq 0) {
        return @{ IsActive = $false; Reason = "No PID" }
    }
    
    try {
        # Try to get process details
        $process = Get-Process -Id $PythonPID -ErrorAction Stop
        
        # Check 1: Process hasn't exited
        if ($process.HasExited) {
            return @{ IsActive = $false; Reason = "Process has exited" }
        }
        
        # Check 2: CPU usage - if it's using CPU, it's probably working
        # Note: This requires WMI and might be heavy, so we'll use it sparingly
        $cpuUsage = $null
        try {
            $cpuCounter = "\Process(python*)\% Processor Time"
            $cpu = Get-Counter -Counter $cpuCounter -ErrorAction SilentlyContinue
            if ($cpu) {
                $cpuUsage = $cpu.CounterSamples[0].CookedValue
            }
        } catch {
            # CPU check failed, but process is still running
        }
        
        # Check 3: Check if process has been responsive recently
        $isHung = $false
        $hangThreshold = 300  # 5 minutes of no activity
        
        if ($global:PythonStartTime) {
            $runTime = (Get-Date) - $global:PythonStartTime
            if ($runTime.TotalSeconds -gt $hangThreshold) {
                # Check if we have recent log file updates
                $logActivity = $false
                if ($CurrentRunFolder) {
                    $logPath = Join-Path $CurrentRunFolder "logs"
                    if (Test-Path $logPath) {
                        $latestLog = Get-ChildItem -Path $logPath -Filter "*.log" -ErrorAction SilentlyContinue | 
                                      Sort-Object LastWriteTime -Descending | 
                                      Select-Object -First 1
                        if ($latestLog -and ((Get-Date) - $latestLog.LastWriteTime).TotalSeconds -lt 300) {
                            $logActivity = $true
                        }
                    }
                }
                
                if (-not $logActivity -and (-not $cpuUsage -or $cpuUsage -eq 0)) {
                    $isHung = $true
                }
            }
        }
        ...
        return @{
            ...
            TotalRuntime = if ($process.StartTime) { (Get-Date) - $process.StartTime } else { $null }
        }
        
    } catch [System.ArgumentException] {
        # Process doesn't exist
        return @{ IsActive = $false; Reason = "Process not found" }
    } catch {
        return @{ IsActive = $false; Reason = "Error checking process: $_" }
    }
}
...
function Collect-RunFolderAttachments {
    ...
    $completionSummaryPath = Join-Path $RunFolder "Laptop_Process_*\logs\completion_summary.txt"
...
function Start-WorkerProcess {
    ...
    try {
        ...
        if ($WorkerProcess.Start()) {
            ...
            $maxWaitTime = 210  # Increased from 60 to 210 seconds
            $waitInterval = 2
            $waited = 0
...
function Get-PythonRuntime {
    if (-not $PythonStartTime -or -not $PythonIsRunning) {
        return $null
    }
    
    try {
        $runtime = (Get-Date) - $PythonStartTime
        return @{
            TotalSeconds = $runtime.TotalSeconds
            TotalMinutes = $runtime.TotalMinutes
            TotalHours = $runtime.TotalHours
            Formatted = "{0}h {1}m {2}s" -f [math]::Floor($runtime.TotalHours), 
                                            [math]::Floor($runtime.TotalMinutes % 60), 
                                            [math]::Floor($runtime.TotalSeconds % 60)
        }
    } catch {
        return $null
    }
...
function Check-ProcessStatus {
    $status = @{
        WorkerRunning = $false
        PythonRunning = $false
        PythonActive = $false  # New: Whether Python is actually processing
        WorkerPID = $WorkerPID
        PythonPID = $PythonPID
        PythonActivity = $null
    }
    ...
    if ($PythonPID -and $PythonPID -ne 0) {
        ...
        # Then check if it's actually active/processing
        if ($status.PythonRunning) {
            ...
            if ($status.PythonActivity) {
                $activityColor = if ($status.PythonActive) { "Green" } else { "Yellow" }
                Write-Host "  Activity: $(if ($status.PythonActive) {'ACTIVE'} else {'INACTIVE/HUNG'})" -ForegroundColor $activityColor
                
                if ($status.PythonActivity.CPUUsage -ne $null) {
                    Write-Host "  CPU: $([math]::Round($status.PythonActivity.CPUUsage, 1))%" -ForegroundColor White
                }
            }
            ...
            if ($runtime) {
                Write-Host "  Runtime: $($runtime.Formatted)" -ForegroundColor White
            }
        } else {
    ...
    if ((-not $status.PythonRunning) -and $WorkerIsRunning) {
        ...
        if ($foundPID) {
            ...
            if ($status.PythonRunning) {
                $activity = Test-PythonProcessActivity -PythonPID $foundPID
                $status.PythonActivity = $activity
                $status.PythonActive = $activity.IsActive
            }
    ...
    $previousPythonActive = $global:PythonIsActive  # New tracking variable
    
    # Update global state
    $global:WorkerIsRunning = $status.WorkerRunning
    $global:PythonIsRunning = $status.PythonRunning
    # Add this to the tracking variables:
    $global:PythonIsActive = $false
    $global:LastPythonActivityCheck = $null
    $global:PythonInactivityCount = 0
    
    # Detect unexpected stops - but only if Python was actually active before
    if (($previousWorkerRunning -and -not $status.WorkerRunning) -or 
        ($previousPythonActive -and -not $status.PythonActive)) {  # Changed condition
        
        Write-Log "Detected process activity change. Worker: $previousWorkerRunning -> $($status.WorkerRunning), Python Active: $previousPythonActive -> $($status.PythonActive)" -Level "WARN"
        ...
        if ($inWindow -and $CurrentRunFolder) {
            Write-Log "Validating run due to process activity change in active window..." -Level "INFO"
            Validate-And-Collect-OnStop -StopReason "Process Activity Change"
        }
    }
...
try {
    ...
    while ($true) {
        ...
        if ($processStatus.PythonRunning -and $CurrentRunFolder -and 
            ...
            $completionPath = Join-Path $CurrentRunFolder "Laptop_Process_*\logs\completion_summary.txt"
        ...
        try {
            ...
            if ($inWindow -and !$processStatus.PythonActive) {  # Changed from !$processStatus.PythonRunning
                Write-Log "Check: $currentTime | Window: Active | Python Active: $($processStatus.PythonActive)" -Level "DEBUG"
                
                # Check if Python is running but just inactive (hung)
                if ($processStatus.PythonRunning -and !$processStatus.PythonActive) {
                    Write-Log "Python process exists but appears inactive/hung. Runtime: $([math]::Round($processStatus.PythonActivity.TotalRuntime.TotalMinutes, 1)) minutes" -Level "WARN"
                    
                    # Check if it's been hung for too long
                    if ($processStatus.PythonActivity.TotalRuntime.TotalMinutes -gt $Config.MaxPythonRuntimeMinutes) {
                        Write-Log "Python process has exceeded max runtime ($($Config.MaxPythonRuntimeMinutes) minutes). Stopping..." -Level "WARN"
                        Stop-WorkerProcess
                        Start-Sleep -Seconds 10
                    } else {
                        Write-Log "Python process may be processing slowly. Will wait before considering restart." -Level "INFO"
                        continue  # Skip starting new worker, wait longer
                    }
                }
                
                Write-Log "Time window active and no active Python process - starting worker..." -Level "INFO"
                
                # Add delay to prevent rapid restart loops
                if ($LastWorkerAttempt -and ((Get-Date) - $LastWorkerAttempt).TotalSeconds -lt $Config.WorkerStartCooldown) {
                    $remaining = [math]::Round($Config.WorkerStartCooldown - (Get-Date - $LastWorkerAttempt).TotalSeconds)
                    Write-Log "Skipping worker start - too soon after last attempt ($remaining seconds remaining)" -Level "DEBUG"
                    continue
                }
                ...
                if ($started) {
                    ...
                    # Wait longer for Python to start if needed
                    if (!$processStatus.PythonRunning) {
                        Write-Log "Python process not yet started, waiting up to 60 seconds..." -Level "INFO"
                        $waitCount = 0
                        while ($waitCount -lt 30 -and !$processStatus.PythonRunning) {  # 30 * 2 = 60 seconds
                            Start-Sleep -Seconds 2
                            $processStatus = Check-ProcessStatus
                            $waitCount++
                        }
                    ...
                    Write-Log "Worker failed to start. Will retry after cooldown period." -Level "WARN"
                }
            }
