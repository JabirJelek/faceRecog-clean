...
function Initialize-ProjectPortablePaths {
    ...
    if ($IsWorker) {
        $paths.PythonScript = Join-Path $projectRoot "patterns\algorithm\entry_multi_USED-Magick.py"
...
function Get-DateBasedPath {
    ...
    #>
    param(
        [string]$BasePath = (Join-Path $global:ActiveRoot "logs-running\magick"),
    ...
    if (-not (Test-Path $datePath)) {
        try {
            ...
            Write-Host "Failed to create date-based folder: $_" -ForegroundColor Red
            throw
        }
    }
...
function Write-Log {
    <#
    .SYNOPSIS
    Simple logging function for common module
    #>
    param(
        [string]$Message,
        [string]$Level = "INFO"
