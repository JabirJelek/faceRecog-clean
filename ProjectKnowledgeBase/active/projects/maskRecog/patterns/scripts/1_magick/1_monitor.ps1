...
function Check-ProcessStatus {
    ...
    if ($global:WorkerPID -and $global:WorkerPID -ne 0) {
        $status.WorkerRunning = Is-ProcessRunning -ProcessId $global:WorkerPID -ProcessName $Script:Config.WorkerProcessName
    }
    
    # Check Python process
    if ($global:PythonPID -and $global:PythonPID -ne 0) {
        $status.PythonRunning = Is-ProcessRunning -ProcessId $global:PythonPID -ProcessName $Script:Config.PythonProcessName
    }
    ...
    if ((-not $status.PythonRunning) -and $global:WorkerIsRunning) {
        ...
        if ($foundPID) {
            ...
            $status.PythonRunning = Is-ProcessRunning -ProcessId $foundPID -ProcessName $Script:Config.PythonProcessName
            Save-PIDTracking
        }
    }
...
function Save-PIDTracking {
    ...
    $global:PIDTracking.WorkerPID = $global:WorkerPID
    $global:PIDTracking.PythonPID = $global:PythonPID
    $global:PIDTracking.WorkerStartTime = $workerStartString
    $global:PIDTracking.PythonStartTime = $pythonStartString
    $global:PIDTracking.RunFolder = $global:CurrentRunFolder
    $global:PIDTracking.LastUpdate = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss")
    $global:PIDTracking.EmailQueue = $global:EmailAttachmentQueue
    $global:PIDTracking.CollectedFolders = $global:CollectedRunFolders
    $global:PIDTracking.ShutdownInitiated = $global:IsShuttingDown
    
    try {
        $global:PIDTracking | ConvertTo-Json -Depth 10 | Out-File -FilePath $Script:Config.PIDFilePath -Force
...
function Load-PIDTracking {
    ...
    try {
        ...
        if ($workerAlive -or $pythonAlive) {
            ...
            Write-Log "PID tracking loaded: WorkerPID=$global:WorkerPID (Alive: $workerAlive)" -Level "INFO"
            return $true
        } else {
