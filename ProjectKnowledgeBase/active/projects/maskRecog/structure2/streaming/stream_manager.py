...
class StreamManager:
    ...
    def _initialize_camera(self, source: str, config: Dict) -> bool:
        """Initialize local camera with DirectShow backend for Windows"""
        try:
            camera_id = int(source)
            
            # Use DirectShow backend for Windows to avoid MSMF conflicts
            ...
            self.cap.set(cv2.CAP_PROP_FPS, config.get('fps', 15))  # Reduced FPS
    ...
    def read_frame(self) -> Tuple[bool, Optional[np.ndarray]]:
        """
        Read frame from stream with enhanced MSMF error handling.
        """
        with self.lock:
            ...
            try:
                ...
                if not ret:
                    ...
                    if error_code == -1072873821:  # MSMF network read error
                        ...
                        if self._attempt_capture_reset():
                            return False, None
    ...
    def _attempt_capture_reset(self) -> bool:
        """Attempt to reset the capture object without full reconnection"""
        try:
            if self.cap is not None:
                # Release and recreate the capture object
                self.cap.release()
