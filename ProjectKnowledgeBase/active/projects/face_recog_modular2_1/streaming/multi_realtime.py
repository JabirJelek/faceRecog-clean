...
# streaming/multi_realtime.py - Updated __init__ method

class MultiSourceRealTimeProcessor:
    def __init__(self, face_system, config: Dict):
        super().__init__(face_system, config)
        
        # 🆕 HEADLESS MODE FLAG - Centralized control
        self.headless_mode = config.get('headless_mode', False)
        print(f"🎯 {'🚫 HEADLESS MODE' if self.headless_mode else '🖥️  WINDOWED MODE'} initialized")
        ...
        # 🆕 CONDITIONAL Display management
        if not self.headless_mode:
            # Only initialize display components in windowed mode
            ...
            self.max_display_sources = None
            self.resizer = None
            print("📊 Display components SKIPPED in headless mode")
        
        # Initialize modular components (always needed)
        ...
        self.show_performance_stats = not self.headless_mode  # Show in console for headless
        ...
        # Stream recovery and health monitoring
        ...
        if self.headless_mode:
            self._setup_headless_optimizations()                            
                                
    def _setup_headless_optimizations(self):
        """Configure optimizations for headless mode"""
        # Use faster processing interval for headless (process more frames)
        ...
        # Reduce logging overhead for headless
        self.log_interval = max(self.log_interval, 2)  # At least every 2 frames
        
        # Optimize face system for performance if available
        if hasattr(self.face_system, 'set_high_performance_mode'):
            self.face_system.set_high_performance_mode()
        
        print("🚀 Headless optimizations applied:")
        print("   - Processing every frame")
        print("   - Aggressive dynamic scaling")
        print("   - High-performance face system mode")                                   
