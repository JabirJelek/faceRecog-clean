...
Propose creation of abstract loader, so that each config and each module can be utilized accordingly
...
$RUN*FOLDER_PREFIX = "Magick_Process_MaskDetect*"
$LOG_SUBFOLDER_NAME = "logs"
$OUTPUT*SUBFOLDER_NAME = "script_output"
$METADATA_FILENAME = "metadata.json"
$PYTHON_OUTPUT_FILENAME_PREFIX = "python_output*"
...
function Load-HarmonizationConfig {
[CmdletBinding()]
param(
[string]$ConfigPath = "config_harmonization.json"
)
    ...
    if (Test-Path $configFullPath) {
        try {
            $config = Get-Content $configFullPath -Raw | ConvertFrom-Json -AsHashtable
            Write-Host "Harmonization configuration loaded from: $configFullPath" -ForegroundColor Green
            return $config
        } catch {
            Write-Host "Warning: Failed to load harmonization config: $_" -ForegroundColor Yellow
            return $null
        }
    } else {
        Write-Host "Warning: Harmonization config not found at: $configFullPath" -ForegroundColor Yellow
        return $null
    }
...
$harmonizationConfig = Load-HarmonizationConfig
if ($harmonizationConfig) {
$COMMON_PATHS_SCRIPT_NAME = "1_common-paths.ps1" # Keep as is, uses common-paths

    if ($harmonizationConfig.mask_portable) {
        $RUN_FOLDER_PREFIX = $harmonizationConfig.mask_portable.run_folder_prefix
        $LOG_SUBFOLDER_NAME = $harmonizationConfig.mask_portable.log_subfolder_name
        $OUTPUT_SUBFOLDER_NAME = $harmonizationConfig.mask_portable.output_subfolder_name
        $METADATA_FILENAME = $harmonizationConfig.mask_portable.metadata_filename
        $PYTHON_OUTPUT_FILENAME_PREFIX = $harmonizationConfig.mask_portable.python_output_filename_prefix
        $COMPLETION_SUMMARY_FILENAME = $harmonizationConfig.mask_portable.completion_summary_filename
        $PYTHON_ARGUMENT = $harmonizationConfig.mask_portable.python_argument
    }
